...
app.listen(port, () => {
  ...
  // One-time cleanup: remove auto-generated VISITA schedule entries (visits no longer sync to schedule)
  const { getPrisma } = require('./db')
  getPrisma().scheduleEntry.deleteMany({ where: { taskType: 'VISITA', isAutoGenerated: true } })
    .then(r => { if (r.count > 0) console.log(`[startup] Removed ${r.count} VISITA schedule entries`) })
    .catch(() => {})
})
