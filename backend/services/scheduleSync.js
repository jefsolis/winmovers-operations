...
const { notifyScheduleAttention } = require('./notifications')
...
async function syncJobScheduleEntries(job, req = null, { forceOverride = false, overrideReason = null } = {}) {
  ...
  if (startDateRaw) {
    existingEntry = await db.scheduleEntry.findFirst({ where: matchWhere })
    if (!job.personalCount) {
      ...
      const { fits } = await checkCapacityForSpan(startDateRaw, daysToComplete, job.personalCount, existingEntry?.id || null)
      if (fits) {
      ...
      } else if (existingEntry?.needsAttention) {
        // Already flagged from a prior override — keep it flagged with its stored reason rather than re-blocking.
        needsAttention = true
        resolvedOverrideReason = existingEntry.overrideReason
      } else {
        const suggestions = await findClosestAvailableSpan(startDateRaw, daysToComplete, job.personalCount, existingEntry?.id || null)
        warning = {
          code: 'NO_CAPACITY',
  ...
  if (!warning) {
    const scheduledEntry = await _syncEntry(db, {
      matchWhere,
      date:         startDateRaw,
      days:         daysToComplete,
    ...
    if (needsAttention && !existingEntry?.needsAttention && scheduledEntry) {
      notifyScheduleAttention({
        entryId: scheduledEntry.id,
        jobId: job.id,
        startDate: scheduledEntry.startDate,
        endDate: scheduledEntry.endDate,
        overrideReason: resolvedOverrideReason,
      }).catch(() => {})
    }
  }
